...
oc get machineset $MACHINE_SET_WORKER_NAME -n openshift-machine-api -o yaml > ../helm-chart/templates/gpu/machineset-gpu.yaml
...
sed -i '' \
    ...
    "../helm-chart/templates/gpu/machineset-gpu.yaml"


# Step 2: Get cluster id and store value
CLUSTER_ID=$(oc get clusterversion version -o json | jq -r '.spec.clusterID')
echo "CLUSTER_ID: \"$CLUSTER_ID\"" >> ../helm-chart/values.yaml
